...
// Расписание намазов для Симферополя на 2026 год (по данным qmdi.ru и ДУМ Крыма)
...
// Функция для получения времени намаза на сегодня
export function getTodayPrayers(): DailyPrayers | null {
  const today = new Date().toISOString().split('T')[0];
  return prayerTimes2026[today] || null;
}

// Функция для получения времени намаза на конкретную дату
