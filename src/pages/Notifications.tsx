  ...
  const requestPushPermission = async () => {
    ...
    try {
      ...
      if (error.message === 'PushManager_Not_Supported') {
        ...
        toast.error(`${t('notifications.toast.error_prefix')}${error.message || t('notifications.toast.error_default')}`);
      }
    } finally {
      ...
      <div className="bg-white px-4 py-4 border-b border-gray-200 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {unreadCount > 0 && (
              <button onClick={markAllRead} className="p-2 text-gray-400 hover:text-emerald-600 transition-colors" title={t('notifications.mark_all_read')}>
                <CheckCheck className="w-5 h-5" />
              </button>
            )}
            {readCount > 0 && (
              <button onClick={clearRead} className="p-2 text-gray-400 hover:text-rose-600 transition-colors" title={t('notifications.clear_read')}>
              ...
              <div className="flex-1">
                ...
                {isIOS && !isStandalone ? (
                  <div className="mt-4 bg-amber-50 border border-amber-100 rounded-xl p-3 flex items-start gap-3">
                    ...
                    <p className="text-xs text-amber-800 leading-normal" dangerouslySetInnerHTML={{ __html: t('notifications.iphone_instruction') }} />
                  </div>
                ) : (
                  <button 
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-0.5">
                          {t(`notifications.type_labels.${group.type}`, { defaultValue: t('notifications.default_label') })}
                        </span>
                      ...
                      <div className="mt-2 space-y-2">
                        {group.items.slice(0, 3).map((item) => (
                          <div key={item.id} className="flex items-start gap-2">
                            ...
                            <p className="text-xs text-gray-500 line-clamp-1">{item.body || t('notifications.no_description')}</p>
                          </div>
                        ))}
